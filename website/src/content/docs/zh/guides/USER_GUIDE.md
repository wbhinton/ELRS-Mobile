...
## 概述
此应用程序提供了一个全面的移动界面，用于管理 ExpressLRS 硬件。它使用户能够直接从移动设备配置接收器 (RX) 和发射器 (TX) 参数、映射 PWM 输出以及刷写固件更新。
...
### 手动安装
对于高级用户或喜欢手动管理的用户，请访问我们的 [GitHub Releases](https://github.com/wbhinton/ELRS-Mobile/releases) 页面获取最新的 `.apk` 文件。
...
3. 将手机的 Wi-Fi 连接到 **ExpressLRS RX** 或 **ExpressLRS TX** 网络 (密码: `expresslrs`)。
4. 打开应用程序。Dashboard 将使用 mDNS 自动扫描网络并连接到设备。

### 阶段 3：刷写固件
1. 从 Dashboard 中，选择 **刷写设备**。
2. **选择您的硬件**：使用级联下拉菜单选择您的 Device Type、Vendor、Frequency 和 Target。
3. **配置选项**：设置您的自定义 Binding Phrase、Wi-Fi 凭据和 Regulatory Domain。
4. 点击 **FLASH**。应用程序将在本地组装 binary 并将其推送到设备。
...
3. **手动 IP 覆盖**：对于复杂的网络，您可以在 Settings 中手动输入 IP 地址。

**强制网络绑定**：现代智能手机通常会丢弃没有互联网的 Wi-Fi 连接。ELRS Mobile 包含一个原生的“强制路由”层，强制操作系统保持硬件连接，确保在现场 100% 可靠的发现。

---

## 专家模式与审计
在 Settings 菜单中，用户可以启用 **专家模式**。这允许您将组��好的固件 binary 直接保存到您的设备，以便通过 FTDI 进行审计或手动刷写。
...
<div class="p-4 my-8 rounded-2xl border border-primary/20 bg-surface/50 backdrop-blur-md shadow-xl shadow-primary/5">
  ...
  <p class="text-sm leading-relaxed text-text-muted/90 pl-11">
    虽然 ELRS Mobile 可以为 STM32 targets (高达 ELRS v3.x) 组装和修补固件，但这些设备不支持通过 Wi-Fi 进行 Over-The-Air (OTA) 刷写。如果您选择一个 STM32 target，应用程序将允许您在本地构建和保存 binary，以便您可以通过 STLink 或 Betaflight Passthrough 使用 PC 手���刷写它。
  </p>
</div>

对于希望验证这些 binaries 完整性的开发者，请参阅我们的 [**固件验证指南**](/guides/firmware-validation)。
