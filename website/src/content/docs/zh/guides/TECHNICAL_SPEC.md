...
<div class="p-4 my-8 rounded-2xl border border-primary/20 bg-surface/50 backdrop-blur-md shadow-xl shadow-primary/5">
  ...
  <p class="text-sm leading-relaxed text-text-muted/90 pl-11">
    该应用程序使用 Flutter 构建，并利用 <strong>Riverpod</strong> 状态管理框架。它通过设备板载 WiFi 模块公开的 RESTful API 与 ELRS 硬件交互，确保低延迟通信和实时状态同步。
  </p>
</div>
...
| `POST` | `/options.json` | 更新可修改的运行时选项（SSID、密码等）。 |
...
- `options`：可修改的用户偏好和网络凭据。
- `config`：低级硬件配置（协议，PWM 数组）。
...
## 映射层
`ElrsMappings.domains900` 将 API 中使用的 900 MHz 监管域索引映射到其人类可读的标签：
...
## 持久化层
系统实现双层持久化策略：
- **`SharedPreferences`**：通过 `PersistenceService` 用于存储非敏感数据，例如 WiFi SSID 和一般应用程序偏好设置。
- **`FlutterSecureStorage`**：用于存储敏感数据，包括绑定短语和 WiFi 密码，确保操作系统级别的加密。
